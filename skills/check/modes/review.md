...
- `TESTS = none-by-design`: `test-preferences.json` records a `"gate"` (e.g. `typecheck+verify`) with no framework, or the nearest `AGENTS.md`/governing spec states a "no test runner" convention. Deliberate: the gate is typecheck + `/check verify`, not a suite.
