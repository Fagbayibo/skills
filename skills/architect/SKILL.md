...
**After all stages are signed off:** the confirmed acceptance criteria seed the ADR's `## Requirements`; the confirmed data model, API surface, and stack **derive `## Build plan`** (each task tagged with the AC it satisfies, the migration first). Then spawn the subagent (below).
