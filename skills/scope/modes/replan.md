...
1. Re-read the whole scope (single file, or `index.md` + epics; the workspace's in a monorepo) and the code/specs for what just shipped.
...
6. Report via the completion block (mode: replan): marked done, enrolled from spec follow-ups, reordered/dropped, next step.
