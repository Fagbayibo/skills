...
- **Workflow tier** (only when it differs from the project default set in Step 5b): `Quick` / `Standard` / `Thorough` / `Full`, from this feature's risk, scope, and compliance sensitivity. Most features inherit the project default; tag a feature (e.g. `· Full`) only when it warrants more or less rigor than the rest. Higher tier → more likely `Needs spec: yes`.
...
- **Needs spec?**: the invent test: would building it require a decision the engineer hasn't made? Yes for a provider/library choice, a data model, a cross cutting pattern, the design system, a whole page/screen with no spec yet, or behavior that is not trivial (search, filtering, recommendations). No only for genuinely pure implementation an existing `design.md`/spec/convention covers. Unsure → yes; an unflagged decision is the expensive miss. `Full`/`Thorough` tier → almost always yes.
...
Depth governs only the stages **after** `/develop` (verify, test, review, document). It does **not** turn off the `/architect` gate: at every depth, a feature that needs a load bearing decision still runs `/architect` first (or records an `Assumed` spec). Standard does not mean "skip architect"; it means lean features are usually `Needs spec: no`, so you rarely reach it.
...
- **options** (mark exactly one `(recommended)` by the product signals, one line why):
  - `Quick`: "Just `/develop`. Nothing after it, you rely on `/develop`'s own build time self check (typecheck, and rendering the screen when it can) and your own eye. No `/check verify`, no test suite, no review. `/develop` can mark the feature `done` itself. Best for throwaway prototypes, experiments, and personal projects."
  - `Standard`: "After `/develop`, `/check verify` on the real app. No separate test suite or second model review unless a feature needs it. Best for low risk features and internal tools you still want proven."
  - `Thorough`: "After `/develop`, `/check verify` then `/test`. No fresh model review by default. Best for most real products."
...
The picker appends a free text Other automatically; in a plain text fallback offer the same four. Recommend by signal: throwaway prototype, experiment, or a personal one off → `Quick`; a low risk product or internal tool you still want proven → `Standard`; a normal production product → `Thorough`; payments, auth, PII, compliance, regulated, or a team codebase → `Full`.

Each tier also sets what `done` means (see `scope-template.md`): `Quick` closes on build plus self check (`/develop` marks it), `Standard` on `/check verify` passing, `Thorough`/`Full` on the tests being in (and the review for `Full`). The `Assumed` spec done gate still holds at every tier: a feature built on an unratified assumption cannot be `done` until `/architect` ratifies it.
