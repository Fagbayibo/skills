...
Workflow tier (one rigor dial per feature, `Quick` · `Standard` · `Thorough` · `Full`): how much process a feature warrants, replacing the older separate "weight". It is a recommendation the engineer overrides, never a forced track. There is ONE project default (recommended once, plan Step 5b, recorded on the scope header `**Workflow:**` line) and a per feature override (a tag beside the heading, e.g. `· Full`, only when a feature differs from the default; no tag = inherit). What the tier drives:
- **Design time**: higher tier → more likely `Needs spec: yes`, and the spec's cross model decision critic runs (auto at `Full`/`Thorough`). `Quick`/`Standard` features are often `Needs spec: no`.
- **After `/develop`** (the verification tail): `Quick` = nothing (rely on `/develop`'s own build time self check); `Standard` = `/check verify`; `Thorough` = `/check verify` + `/test`; `Full` = adds a fresh model `/check review` + `/document`.
- **What closes `done`** (the last required stage marks it): `Quick` → `/develop` (build + self check); `Standard` → `/check verify`; `Thorough`/`Full` → `/test`. An `Assumed` spec still blocks `done` at every tier.

`/scope` recommends the project default from the risk and size of the feature mix (Step 5b), the same signals `/architect` and `/develop` read; `/develop` reads the effective tier (feature override, else project default, else inferred) to scale the next steps it recommends, so a `Quick` project is not told to run verify and a `Standard` project is not told to run a full review chain. Not a global playbook a feature is slotted into: it is a per project, per feature judgment.
