...
Spawn with `model` set explicitly to a strong model (do not inherit the session model; Claude Code: `sonnet`, reserving `opus` for an unusually complex suite); `description: "Test: <tool> suite for <N> changed files"`; tools `Read`, `Bash`, `Write`, `Edit`. The prompt contains:
